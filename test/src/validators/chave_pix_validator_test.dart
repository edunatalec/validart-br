...
void main() {
  ...
  group('ChavePixValidator — defaults (5 chaves DICT, sem BR Code)', () {
    test('aceita CPF válido sem máscara', () {
      final VString schema = V.string()..add(const ChavePixValidator());
    ...
    test('retorna código de erro invalid_pix_key', () {
      final VString schema = V.string()..add(const ChavePixValidator());
      final List<VError>? errors = schema.errors('not a key');
  ...
  group('ChavePixValidator — tipos restritos', () {
    test('tipos: [email] aceita só e-mail', () {
      final VString schema = V.string()
    ...
    test('tipos: [cpf, cnpj] aceita só documentos', () {
      final VString schema = V.string()
        ..add(
    ...
    test('tipos: [random] aceita só UUID', () {
      final VString schema = V.string()
    ...
    test('atalho V.string().chavePix(tipos: …) propaga o filtro', () {
      final VString schema = V.string().chavePix(
  ...
  group('ChavePixValidator — BR Code (tipos inclui brCode)', () {
    ...
    test('rejeita BR Code com CRC alterado (1 char)', () {
      ...
      final String quebrado = _brCodeUuidComValor.replaceRange(
    ...
    test('rejeita BR Code truncado (CRC some)', () {
      final VString schema = V.string()..add(validator);
      final String truncado = _brCodeUuidComValor.substring(
        0,
    ...
    test('rejeita BR Code sem campo "6304" antes do CRC', () {
      ...
      final String corrompido = _brCodeUuidComValor.replaceRange(
  ...
  group('ChavePixValidator — união de chaves DICT + BR Code', () {
    test('tipos: TipoChavePix.values aceita chave e BR Code', () {
      final VString schema = V.string().chavePix(tipos: TipoChavePix.values);
