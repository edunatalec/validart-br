...
void main() {
  ...
  group('BrPhonePattern', () {
    group('defaults (DDI opcional, DDD opcional, any mode)', () {
      test('aceita celular com DDD', () {
        final schema = V.string().phone(patterns: [const BrPhonePattern()]);
    ...
    group('countryCode: required', () {
      test('aceita com +55', () {
        final schema = V.string().phone(
          patterns: [
            const BrPhonePattern(countryCode: CountryCodeFormat.required),
          ],
        );
        expect(schema.validate('+55 11 98765-4321'), isTrue);
      });
    ...
    group('countryCode: none', () {
      test('rejeita com +55', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(countryCode: CountryCodeFormat.none)],
        );
    ...
    group('areaCode: required', () {
      test('aceita com DDD', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(areaCode: AreaCodeFormat.required)],
        );
        expect(schema.validate('11987654321'), isTrue);
      });
    ...
    group('areaCode: none', () {
      test('rejeita com DDD', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(areaCode: AreaCodeFormat.none)],
        );
        expect(schema.validate('(11) 98765-4321'), isFalse);
      });
    ...
    group('mobileOnly', () {
      test('aceita celular', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(mobileOnly: true)],
        );
        expect(schema.validate('11987654321'), isTrue);
      });
    ...
    group('mode', () {
      test('formatted rejeita só dígitos', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(mode: ValidationMode.formatted)],
        );
      ...
      test('unformatted rejeita com separadores', () {
        final schema = V.string().phone(
          patterns: [const BrPhonePattern(mode: ValidationMode.unformatted)],
        );
        expect(schema.validate('(11) 98765-4321'), isFalse);
        expect(schema.validate('11987654321'), isTrue);
      });
